...
public class CtrlAltDefeat implements BotAPI {
    ...
    public String getCommand(Plays possiblePlays) {
    	String cmd;
    	if(getCurrentBoardState() <= condition && getCurrentBoardState() >= -100 && match.canDouble(me.getId()) && canBearOff() == false) {
    		cmd = "double";
    		condition -= 30;
    	}
    	else if(getCurrentBoardState() >= -100) {
    		condition = -50;
	    	if(canBearOff()) {
    	...
    	System.out.println("Current Board Vaslue: " + getCurrentBoardState());
    	return cmd;
    }
    
    public double getCurrentBoardState() {
    	return getEvaluation(board.get(),1,0,0,0,0,0,0,0);
    }
    ...
    public int getMove(Plays possiblePlays) {
        ...
        for(int i = 0;i < possiblePlays.number();i++) {
            ...
            probability.add(getEvaluation(checkerLayoutAfterMove,14,4,17,10,14,14,7,3));  // weights can be altered here
        }
    ...
    {
    	String playDouble;
    	double probability = getEvaluation(board.get(),1,0,0,0,0,0,0,0);
